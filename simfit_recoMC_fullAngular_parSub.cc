...
void simfit_recoMC_fullAngularBin(int q2Bin, int parity, bool multiSample, uint nSample, bool plot, bool save, std::vector<int> years, std::map<int,float> scale_to_data, double fac1, double fac4, double fac5, double base1, double base4, double base5, double max1, double max4, double max5)
{
  ...
  TFile* fout = new TFile(("simFitResult_recoMC_fullAngular" + all_years + stat + Form("_b%i.root", q2Bin)).c_str(),"UPDATE");
...
//       std::cout << "dataset: " << it->first << ", with n entries: " << it->second->sumEntries() << "\n";
  ...
  if (multiSample) {
    ...
    cDist->cd();
    fDist->Draw();
    cDist->SaveAs( ("recoBoundDist11_" + shortString + "_" + all_years + Form("_f-%.3f-%.3f-%.3f_b-%.3f-%.3f-%.3f_m-%.0f-%.0f-%.0f.pdf",fac1,fac4,fac5,base1,base4,base5,max1,max4,max5)).c_str() );
  }
  ...
  cnll->Update();
  cnll->SaveAs( ("recoNLL_scan_" + plotString + ".pdf").c_str() );

  cZoom->Update();
  cZoom->SaveAs( ("recoNLL_scan_" + plotString + "_zoom.pdf").c_str() );

  cPen->Update();
  cPen->SaveAs( ("recoPenTerm_" + plotString + ".pdf").c_str() );
  return;
  ...
  c[confIndex]->SaveAs( ("simFitResult_recoMC_fullAngular_" + shortString + "_" + all_years + stat + ".pdf").c_str() );
