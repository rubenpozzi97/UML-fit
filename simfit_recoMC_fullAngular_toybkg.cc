...
//   RooArgSet reco_vars (*ctK, *ctL, *phi, *rand);
  ...
  for (unsigned int iy = 0; iy < years.size(); iy++) {
    ...
    // create toy bkg in angular variables 
    RooRealVar *c1 = new RooRealVar("c1", "c1", 1, 0, 10);
    RooRealVar *c2 = new RooRealVar("c2", "c2", 20, 0.1, 10);
    RooRealVar *c3 = new RooRealVar("c3", "c3", 5, 0.1, 10);
    RooArgList coeffs1(*c1,*c2,*c3);
 
    RooRealVar *d1 = new RooRealVar("d1", "d1", 12, 0, 10);
    RooRealVar *d2 = new RooRealVar("d2", "d2", 2, 0.1, 10);
    RooRealVar *d3 = new RooRealVar("d3", "d3", 5, 0.1, 10);
    RooArgList coeffs2(*d1,*d2,*d3);
    
    RooRealVar *e1 = new RooRealVar("e1", "e1", 1, 0, 10);
    RooRealVar *e2 = new RooRealVar("e2", "e2", 2, 0.1, 10);
    RooRealVar *e3 = new RooRealVar("e3", "e3", 6, 0.1, 10);
    RooArgList coeffs3(*e1,*e2,*e3);
    ...
    RooBernstein* b1 = new RooBernstein("b1", "b1", *ctK, coeffs1);
    RooBernstein* b2 = new RooBernstein("b2", "b2", *ctL, coeffs2);
    RooBernstein* b3 = new RooBernstein("b3", "b3", *phi, coeffs3);

    RooProdPdf *bkg_pdf = new RooProdPdf("final", "final", RooArgList(*b1,*b2,*b3));
    
      cout << "firstSample: " << firstSample << "  " << lastSample << endl;
    for (uint itoy = 0; itoy < lastSample+1-firstSample; itoy++){
      RooDataSet *toy_bkg = bkg_pdf->generate(reco_vars, 100, Extended(true));
    
      cout << "before: " << data[iy][itoy]->sumEntries() << endl;
      data[iy][itoy]->append(*toy_bkg);
      cout << "after: " << data[iy][itoy]->sumEntries() << endl;
    } 
...
//          		                             *ang_rt, *ang_wt
//       		                                     ) );
    PdfSigAng* sig_ang_fullAngular =  new PdfSigAng(("PDF_sig_ang_fullAngular_"+shortString+"_"+year).c_str(),
                                                     ("PDF_sig_ang_fullAngular_"+year).c_str(),
      		                                     *ctK,*ctL,*phi,*mFrac,
         		                             *ang_rt, *ang_wt
      		                                     );
...
// 							     *penTerm));
    ...
    PdfSigAng* sig_ang_fullAngular_penalty = new PdfSigAng(("PDF_sig_ang_fullAngular_penalty_"+shortString+"_"+year).c_str(),
							     ("PDF_sig_ang_fullAngular_penalty_"+year).c_str(),
							     *ctK,*ctL,*phi,*mFrac,
         		                                     *ang_rt, *ang_wt,
							     *penTerm);

    RooRealVar *fsig = new RooRealVar( ("nsig"+shortString+"_"+year).c_str(), ("nsig"+shortString+"_"+year).c_str(),0,1 );
//     RooRealVar *nbkg = new RooRealVar( ("nbkg"+shortString+"_"+year).c_str(), ("nbkg"+shortString+"_"+year).c_str(),0,10000 );
    RooAddPdf* full_pdf = new RooAddPdf( ("PDF_sig_ang_fullAngular_bkg_"+shortString+"_"+year).c_str(),
                                         ("PDF_sig_ang_fullAngular_bkg_"+shortString+"_"+year).c_str(),
                                          RooArgList(*sig_ang_fullAngular, *bkg_pdf),
                                          RooArgList(*fsig)
                                       );
    PDF_sig_ang_fullAngular.push_back(full_pdf);

    RooAddPdf* full_pdf_penalty = new RooAddPdf( ("PDF_sig_ang_fullAngular_bkg_"+shortString+"_"+year).c_str(),
                                                 ("PDF_sig_ang_fullAngular_bkg_"+shortString+"_"+year).c_str(),
                                                 RooArgList(*sig_ang_fullAngular_penalty, *bkg_pdf),
                                                 RooArgList(*fsig)
                                       );
    PDF_sig_ang_fullAngular_penalty.push_back(full_pdf_penalty);
  ...
  TFile* fout = new TFile(("simFitResults/newphi/simFitResult_recoMC_fullAngular" + all_years + stat + Form("_b%i_toybkg.root", q2Bin)).c_str(),"UPDATE");
...
// 	fitter->improveAng();
  ...
  if (multiSample) {
    ...
    cDist->cd();
    fDist->Draw();
    cDist->SaveAs( ("plotSimFit_d/recoBoundDist_" + shortString + "_" + all_years + "_toybkg.pdf").c_str() );
  }
  ...
  cnll->Update();
  cnll->SaveAs( ("plotSimFit_d/recoNLL_scan_" + plotString + "_toybkg.pdf").c_str() );

  cZoom->Update();
  cZoom->SaveAs( ("plotSimFit_d/recoNLL_scan_" + plotString + "_zoom_toybkg.pdf").c_str() );

  cPen->Update();
  cPen->SaveAs( ("plotSimFit_d/recoPenTerm_" + plotString + "_toybkg.pdf").c_str() );
  ...
  c[confIndex]->SaveAs( ("plotSimFit_d/newphi/simFitResult_recoMC_fullAngular_" + shortString + "_" + all_years + stat + "_toybkg.pdf").c_str() );
