...
void simfit_recoMC_fullAngularBin(int q2Bin, int parity, bool multiSample, uint nSample, bool plot, bool save, std::vector<int> years, std::map<int,float> scale_to_data, double fac1, double fac4, double fac5, double base1, double base4, double base5, double max1, double max4, double max5)
{
  ...
  std::vector<RooGaussian*> c_deltaPeaks, c_frt;
  RooArgSet c_vars, c_pdfs;
  ...
  for (unsigned int iy = 0; iy < years.size(); iy++) {
    ...
    PDF_sig_mass.push_back(new RooProdPdf (("mass_pdf"+year).c_str(),
                                           ("mass_pdf"+year).c_str() , 
                                           constr_list
                                          ) 
                          );          
	...
	simPdf        -> addPdf(*mass_ang_pdf,         ("data"+year+Form("_subs%d",is)).c_str());
	simPdf_penalty-> addPdf(*mass_ang_pdf_penalty, ("data"+year+Form("_subs%d",is)).c_str());
      }
    else {
	...
	return;
      ...
      simPdf        ->addPdf(*mass_ang_pdf,         ("data"+year+Form("_subs%d",firstSample)).c_str());
      simPdf_penalty->addPdf(*mass_ang_pdf_penalty, ("data"+year+Form("_subs%d",firstSample)).c_str());
    }

  }


  TFile* fout = new TFile(("simFitResults4d/simFitResult_recoMC_fullAngularMass" + all_years + stat + Form("_b%i.root", q2Bin)).c_str(),"UPDATE");
...
//   return;
  ...
  for (unsigned int iy = 0; iy < years.size(); iy++) {
    ...
    for (unsigned int fr = 0; fr < frames.size(); fr++){
        ...
        simPdf ->plotOn(frames[fr], Slice(sample, ("data"+year+"_subs0").c_str()), ProjWData(RooArgSet(sample), *combData), LineWidth(1), Name(("plPDF"+year).c_str()));
        if (fr == 0) {
  ...
  c[confIndex]->SaveAs( ("plotSimFit4d_d/simFitResult_recoMC_fullAngularMass_" + shortString + "_" + all_years + stat + ".pdf").c_str() );
