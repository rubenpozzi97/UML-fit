...
void simfit_recoMC_fullAngularBin(int q2Bin, int parity, bool multiSample, uint nSample, bool localFiles, bool plot, bool save, double mF,std::vector<int> years)
{
  ...
  //std::vector<std::vector<double>>mFracdata;
  ...
  RooRealVar* mFrac = new RooRealVar("mFrac","mistag fraction",mF, 0, 2);
  mFrac->setConstant();
  ...
  TFile* fout = new TFile(("simFitResults/simFitResult_recoMC_fullAngular" + all_years + stat + Form("_b%i_mFrac%i.root", q2Bin,mF)).c_str(),"RECREATE");
...
//       std::cout << "dataset: " << it->first << ", with n entries: " << it->second->sumEntries() << "\n";
  ...
  longString = longString + Form(parity==1?" (q2-bin %i even)":" (q2-bin %i odd)",q2Bin) + Form("mFrac = %i",mF);
  ...
  c[confIndex]->SaveAs( ("plotSimFit_d/simFitResult_recoMC_fullAngular_" + shortString + "_" + all_years + stat + "_newphi_" + Form("mFrac=%i.pdf",mF).c_str() );

}


void simfit_recoMC_fullAngularBin1(int q2Bin, int parity, bool multiSample, uint nSample, bool localFiles, bool plot, bool save,double mF, std::vector<int> years)
{
  if ( parity==-1 )
    for (parity=0; parity<2; ++parity)
      simfit_recoMC_fullAngularBin(q2Bin, parity, multiSample, nSample, localFiles, plot, save, mF, years);
  else
    simfit_recoMC_fullAngularBin(q2Bin, parity, multiSample, nSample, localFiles, plot, save, mF, years);
}
...
int main(int argc, char** argv)
{
  ...
  scale_to_data.insert(std::make_pair(2018, 0.007*2 /1.9  ));
    ...
    mFset = generatemF(2016,0); 
    for(auto it = mFset.begin(); it != mFset.end(); ++it){
        double mF = *it;
        cout << "fixed mFrac is " << mF << endl;
        if ( q2Bin==-1 )
        for (q2Bin=0; q2Bin<nBins; ++q2Bin)
            simfit_recoMC_fullAngularBin1(q2Bin, parity, multiSample, nSample, localFiles, plot, save, mF,years);
        else
            simfit_recoMC_fullAngularBin1(q2Bin, parity, multiSample, nSample, localFiles, plot, save, mF,years);
    }
